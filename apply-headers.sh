...
# param 2: template filename, will use templates inside other/spdx/template/$filename
function applySPDXonFirstLine {
    ...
    find ./src -iname \*.$fileEnding | while read file ; do
        ...
        then
            ...
            cat other/spdx/template/$spxTemplate $file >$file.new && mv $file.new $file
        fi
...
function startAutoApply {
    ...
    applySPDXonFirstLine "ts" "spdx_template_doubleslash.txt"
